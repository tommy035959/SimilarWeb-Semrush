...
(function () {
    ...
    // 配置
    const CONFIG = {
        selector: 'div[data-path="report.table"] span[data-ui-name="Link.Text"],a[class="swTable-content"]',
        // Google翻译批量API
    ...
    async function translateAllElements() {
        ...
        // 收集所有需要翻译的文本
        const textsToTranslate = untranslatedElements.map(el => el.textContent.trim());
    ...
    function createTranslateButton() {
        ...
        const filterLayout = document.querySelector('.kwo-one-line-layout,.export-buttons-wrapper,.FiltersContainer');

        if (!filterLayout) {
            console.warn('未找到 .sm-filter-layout 元素，将在3秒后重试');
